...
``payments/test_replay.py`` proves that the *current* workflow code can still
...
2. Builds the real worker (``payments.worker.build_worker``), registering the
...
4. Fetches the resulting execution history from the server and overwrites
   ``payments/testdata/coordinator-history.json`` with it.

The fixture is committed to version control like any other test data. After
regenerating it, re-run ``uv run pytest payments/test_replay.py`` to confirm
...
from payments.workflows import PaymentCorrectionCoordinator  # noqa: E402
...
# The seeded corridor-memory pattern (memory/store.py): this exact
# (corridor, anomaly_type) pair is a guaranteed hit, so capturing it never
# reaches a model — no API key required (the corridor-memory service must be
# running, since read_corridor_memory now calls it over HTTP).
...
def main() -> None:
    ...
    # fixture stores it alongside the history for payments/test_replay.py.
