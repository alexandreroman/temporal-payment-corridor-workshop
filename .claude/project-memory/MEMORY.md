...
- [Workshop slides live in slides/, reveal.js + Temporal theme](references/project_workshop_slides.md) — reveal.js deck, custom Temporal theme, A–G grammar + type-code; Sessions 1 & 2 built, 3 pending.
...
- [Guide screenshots come from real runs, cropped or composed to the caption](references/feedback_guide_screenshots.md) — capture from the live stack, frame to the manifest subject (rounded dark card), compose with magick -append, leave no trace for feature-gated shots; topology is inline Mermaid and CLI output is a Markdown block, not PNGs.
- [Guide: app-first for interactions, explicit UI naming](references/feedback_guide_ui_priority_naming.md) — interact via the app (Approve/Reject) first, keep raw Signal as the Temporal equivalent; write "Temporal Web UI"/"Temporal" vs "the app", never bare "Web UI".
- [Crash & resume demo lives in guide step 02](references/reference_crash_resume_demo.md) — make dev Ctrl-C kills only host processes; Temporal survives; use memory-miss (LLM) for a catchable window, memory-hit is too fast.
