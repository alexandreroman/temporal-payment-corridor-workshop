...
`int(os.getenv(...))` port cast used in `memory/main.py`.

## Logfire is local-only

Three modules — `payments/main_worker.py`, `payments/api.py`, and
`memory/app.py` — call `logfire.configure(...)` with
`send_to_logfire=False`. Spans are produced locally for
